...
def test_integrity(db_conf):
    with sqlite_context("db.sqlite") as sqlite_conn, pg_context(**db_conf) as pg_conn:
...
def check_records(
    sqlite_conn: Connection, pg_conn: _connection, table_name: str
) -> int:
...
def test_consistency(db_conf):
    with sqlite_context("db.sqlite") as sqlite_conn, pg_context(**db_conf) as pg_conn:
        ...
        for table_name in TABLES:
            ...
            ), "Есть неправильно записанные данные"
