...
class FilmService(BaseService):
    def __init__(
        self, cache_service: AsyncCacheStorage, search_service: AsyncSearchStorage
    ):
        self.cache_service = cache_service
        self.search_service = search_service

    @staticmethod
    def transform_obj_data_to_data(
        obj_data: list, elastic_index_name: str
    ...
    async def get_obj_data_by_id(
        ...
        if not obj_data:
            obj_data = await self.search_service.get_by_id(
                obj_id=obj_id, index_name=elastic_index_name
            )
            if not obj_data:
                return None
    ...
    async def get_obj_data_by_query(
        ...
        if not obj_data:
            ...
            if not obj_data:
                logger.debug(
                    "No item in elasticsearch %s in %s index", query, elastic_index_name
                )
                return []
...
def get_redis_elastic_service(
    ...
    elastic_service: AsyncSearchStorage = Depends(get_elastic_service),
) -> BaseService:
