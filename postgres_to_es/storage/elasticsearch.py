...
from storage.elasticsearch_index_schema import (
    MOVIES_INDEX_SCHEMA,
...
class ElasticSaver:
    ...
    def index_create(self):
        if self.index_name == "movies":
            return self.conn.indices.create(
                index=self.index_name, body=self.movies_schema
            )
        elif self.index_name == "genres":
            return self.conn.indices.create(
                index=self.index_name, body=self.genres_schema
            )
        else:
            return self.conn.indices.create(
                index=self.index_name, body=self.person_schema
            )
