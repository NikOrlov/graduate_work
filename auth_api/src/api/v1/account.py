...
from schemas.base import ChangeLogin, ChangePassword, Session, UserInDB
...
@router.post(
    "/logout_me", status_code=status.HTTP_200_OK, description="Выход", tags=["Аккаунт"]
)
async def logout(
...
async def change_password(
    ...
    """
    logger.info(
        "Request to change password for user [%s] with new pass [%s]",
        user.email,
        change_password_data.password,
    )
...
async def logins_history(
    ...
    """
    logger.info(
        "Requst login history for user [%s] with page size [%d]", user.email, page
    )
