...
class RolesService(AsyncRolesService):
    ...
    async def _check_role_by_name(self, name: str) -> Union[User, None]:
        role_exist = await self.db_service.select(
            what_select=Role, where_select=[(Role.name, name)]
        )
        if len(role_exist) > 0:
            return role_exist[0]
        return None
    ...
    async def _check_user_by_email(self, email: str) -> Union[User, None]:
        existing_user = await self.db_service.select(
            what_select=User, where_select=[(User.email, email)]
        )
    ...
    async def create_role(
        ...
        try:
            ...
            if role_exist is not None:
                logger.info("Role already exists: %s", name)
                return HTTPStatus.BAD_REQUEST

            await self.db_service.insert(
                what_insert=Role,
                values_insert={"name": name, "description": description},
            )
    ...
    async def change_role(
        ...
        try:
            ...
            await self.db_service.update(
                what_update=Role, where_update=[Role.name, name], values_update=values
            )
        except Exception as err:
            logging.info(
    ...
    async def get_roles(self) -> Optional[List[Roles]]:
        try:
            ...
            return [
                Roles(name=data.name, description=data.description) for data in datas
            ]
        except Exception as err:
    ...
    async def delete_role(self, name: str) -> Optional[HTTPStatus]:
        try:
            ...
            await self.db_service.delete(
                what_delete=Role, where_delete=[(Role.name, name)]
            )
            return HTTPStatus.ACCEPTED
        except Exception as err:
    ...
    async def set_role_to_user(
        ...
        user_roles = await self.db_service.db.execute(
            select(UserRole).where(
                UserRole.user_id == existing_user.id, UserRole.role_id == role_exist.id
            )
        )
        ...
        data_insert = UserRole(user_id=existing_user.id, role_id=role_exist.id)
    ...
    async def check_permission(self, user: User, role: str):
        if role not in await self.get_user_roles(user):
            logger.info(
                "Role %s does not exist in user [%s] role list", role, user.email
            )
...
def check_admin_permission(func):
    ...
    async def wrapper(
        ...
        **kwargs,
        ...
        return await func(*args, user=user, role_service=role_service, **kwargs)
