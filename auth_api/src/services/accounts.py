...
class AbstractAccountService(AuthService, abc.ABC):
    ...
    @abc.abstractmethod
    async def change_login(self, user: User, change_login_data: ChangeLogin) -> User:
        pass

    @abc.abstractmethod
    async def change_password(
        self, user: User, change_password_data: ChangePassword
    ) -> User:
        pass


class AccountService(AbstractAccountService):
    async def get_token_pairs_with_pagination(
        ...
        sql = (
            ...
            .offset(offset)
            .limit(page_size)
            .order_by(desc(TokenPair.created_at))
        )
    ...
    async def logins_history(self, user: User, page: int) -> list[Session]:
        ...
        if history:
            ...
            raise HTTPException(
                status_code=400,
                detail="Номер страницы больше, " "чем общее количество страниц.",
            )
    ...
    async def logout(self, access_token: str):
        ...
        await self.db_service.update(
            TokenPair, {"logout_at": datetime.utcnow()}, [TokenPair.jti, jti]
        )
    ...
    async def change_login(self, user: User, change_login_data: ChangeLogin) -> User:
        ...
        """
        await self.db_service.update(
            User, {"login": change_login_data.new_login}, [User.id, user.id]
        )
    ...
    async def change_password(
        ...
        await self.db_service.update(
            User, {"password": hash_password}, [User.id, user.id]
        )
