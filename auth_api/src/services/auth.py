...
class AbstractAsyncAuthService(abc.ABC):
    ...
    @abc.abstractmethod
    async def signin(self, form_data: SignInUser, request: Request) -> Tokens:
        pass
...
class AuthService(AbstractAsyncAuthService):
    ...
    async def create_user(self, signup_user: SignUpUser) -> User:
        try:
            ...
            raise HTTPException(
                status.HTTP_409_CONFLICT, "User with login or email already exists."
            )

    async def get_db_user_by_email(self, email: str) -> User:
        result = await self.db_service.select(User, [(User.email, email)])
        if not result:
            logger.error("User does not exist with email %s", email)
            raise HTTPException(
                status.HTTP_401_UNAUTHORIZED, "User with email not found."
            )
        return result[0]

    async def create_db_token_pair(
        self, user_id: str, jti: uuid.UUID, user_agent: Union[str, None]
    ) -> TokenPair:
    ...
    async def signin(
        self, form_data: SignInUser, request: Request
    ) -> SignInUserResponse:
        ...
        if not user.check_password(form_data.password):
            ...
            raise HTTPException(
                status.HTTP_401_UNAUTHORIZED, "Login or Password not valid."
            )
    ...
    def create_token(token_type: str, data: dict, expires_min: int, jti: uuid.UUID):
        ...
        to_encode.update({"jti": str(jti), "exp": expire, "token_type": token_type})
    ...
    async def create_access_token(self, user: User, jti: uuid.UUID) -> str:
        ...
        return self.create_token(
            "access", token_data, settings.ACCESS_TOKEN_EXPIRES_IN, jti
        )
    ...
    async def create_refresh_token(self, user: User, jti: uuid.UUID) -> str:
        ...
        return self.create_token(
            "refresh", token_data, settings.REFRESH_TOKEN_EXPIRES_IN, jti
        )
    ...
    async def create_new_tokens(
        self, user: User, user_agent: Union[str, None]
    ) -> Tokens:
    ...
    async def update_token_pair(self, token: RefreshToken) -> Tokens:
        ...
        await self.db_service.update(
            TokenPair, {"jti": new_jti}, [TokenPair.jti, payload["jti"]]
        )
        return tokens
    ...
    async def is_token_in_deny_list(self, jti: uuid.UUID) -> bool:
        return bool(await self.redis.exists(str(jti)))
    ...
    async def check_token(self, token: str, token_type: str) -> dict:
        ...
        Returns:
            - payload - dict
        """
        error = HTTPException(status.HTTP_401_UNAUTHORIZED, f"{token_type} not valid")
        ...
        if payload["token_type"] != token_type:
            logger.error(
                "JWT error - token_type [%s] != [%s]", payload["token_type"], token_type
            )
            raise error
