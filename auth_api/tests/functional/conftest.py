...
async def db_session() -> AsyncGenerator[AsyncSession, None]:
    engine = create_async_engine(settings.dsl_database, echo=False, future=True)
...
#
#
@pytest_asyncio.fixture(name="make_request_with_session", scope="function")
def make_request_with_session():
    async def inner(
        ...
        async with aiohttp.ClientSession() as session:
            resp = await session.__getattribute__(method)(
                query, data=data, json=json, headers=headers, params=params
            )
            return resp
...
async def admin_auth_data(db_session, make_request_with_session) -> dict:
    ...
    response = await make_request_with_session(
        "post",
        url,
        data={"username": "test_admin@testadmin.com", "password": "test_admin"},
    )
