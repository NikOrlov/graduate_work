...
from tests.functional.test_data.account import (
    CHANGE_LOGIN_DATA,
...
async def test_logout_me(
    ...
    # Act
    await make_request_with_session("post", url, headers=admin_auth_header)
    response = await make_request_with_session("post", url, headers=admin_auth_header)
...
async def test_change_login_fake_user(
    ...
    # Act
    response = await make_request_with_session("put", url, json=CHANGE_LOGIN_DATA)
...
@pytest.mark.parametrize("query_data, expected_answer", LOGIN_DATA_BEFORE_CHANGE_PASS)
@pytestmark
async def test_change_password_fake_user(
    ...
    # Act
    response = await make_request_with_session("post", sign_in_url, data=query_data)
    data_response = await response.json()
    ...
    await make_request_with_session(
        ...
        headers={"Authorization": "Bearer " + data_response.get("access_token")},
    )
    response = await make_request_with_session(
        "post",
        sign_in_url,
        data={"username": query_data["username"], "password": new_pass},
    )
...
async def test_history_user(
    ...
    response = await make_request_with_session(
        "post", url, params={"page": 1}, headers=admin_auth_header
    )
    data_response = await response.json()
