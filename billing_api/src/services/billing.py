...
from models.base import Subscriptions, Payments, PaymentStatus, UserSubscriptions
...
class BillingService:
    ...
    async def _get_pending_payment(
        self, service_payment_id: uuid.UUID, payment_status: str
    ) -> Payments:
        """
    ...
    async def _get_user_subscribe(
        ...
        if not user_subscribe_data:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "user_subscribe not exist")
    ...
    async def _get_subscription(self, subscription_id: uuid.UUID) -> Subscriptions:
        try:
            subscription_data = await self.db.select(
                Subscriptions, where_select=[(Subscriptions.id, subscription_id)]
            )
            return subscription_data[0]
        except Exception:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Подписка с таким id не существует")

    async def _increase_user_subscribe_time(
        ...
        if not user_subscribe_data:
            user_subscribe = await self.db.insert(
                UserSubscriptions,
                {
                    ...
                    + timedelta(subscription.duration * 30),
            ...
            await self.db.update(
                UserSubscriptions,
                {
                    "payment_id": payment_id,
                    "is_active": True,
                    "expiration_time": user_expiration_time
                    + timedelta(subscription.duration * 30),
                },
                [UserSubscriptions.id, user_subscribe.id],
            )

    async def prolongate_subscribe(
        self, user_id: uuid.UUID, user_subscription_id: uuid.UUID
    ) -> bool:
        """
        ...
        if not user_subscribe.auto_prolongate:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Subscribtion not autoprolongate"
            )

        subscription = await self._get_subscription(user_subscribe.subscription_id)
        last_payment_data = await self.db.select(
            Payments, [(Payments.id, user_subscribe.payment_id)]
        )
        last_payment: Payments = last_payment_data[0]

        yookassa_payment = Payment.create(
            {
                "amount": {"value": f"{subscription.price}", "currency": "RUB"},
                "capture": True,
        ...
        payment = await self._create_db_new_payment(
            user_id,
            subscription.id,
            last_payment.service_payment_id,
            subscription.price,
    ...
    async def create_new_payment(
        ...
        await self._create_db_new_payment(
            user_id, subscription_id, yookassa_payment.id, subscription.price
        )

        return yookassa_payment.confirmation.confirmation_url

    async def update_payment_status(
        self, yookassa_payment_id: uuid.UUID, new_status: str
    ):
        """
        Обновляет статус платежа в бд и продлевает подписку в случае успеха
        """
        try:
            payment = await self._get_pending_payment(yookassa_payment_id, "pending")
        except Exception:
            return
        ...
        await self.db.update(
            Payments, {"status": new_status}, [Payments.id, payment.id]
        )
...
def get_billing_service(session=Depends(get_session)) -> BillingService:
