...
from sqlalchemy import Column, DateTime, String, Float, Integer, ForeignKey, Boolean
...
class Subscriptions(UUidMixin, Base):
    ...
    duration = Column(Integer, nullable=False)  # Длительность подписки в месяцах

    payments = relationship("Payments", back_populates="subscription")
    user_subscriptions = relationship(
        "UserSubscriptions", back_populates="subscription"
    )
...
class Payments(UUidMixin, Base):
    ...
    user_subscriptions = relationship("UserSubscriptions", back_populates="payment")
...
class UserSubscriptions(UUidMixin, Base):
    ...
    payment = relationship("Payments", back_populates="user_subscriptions")
